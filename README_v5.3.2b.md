...
- 5.3.2bは現在windows版のみが提供されています.
- 糖鎖リストの糖鎖数は上限4500まで, サンプル数（実験数, Masslistのタブ数）は20に限定されています.ご連絡頂ければ多少の変更はできますが, gfortranの都合で静的に確保できるメモリが限られているのでプログラムの仕様を変更すべく検討中です.

## 1.  変更点
バージョン5.3では動作が大幅に変更になりました. 5.2のようにCSVファイルを用意してTAGexpressionを走らせる機能も残っています. これは後述します. 以下に変更点, 新機能を記載します.

- サマリーファイルを追加しました. 質量順に糖鎖をならべ同じ理論質量をもつ同定された構造は同じセルに出力しました. さらに帰属した糖鎖と残基1つ違いの糖鎖をリスト化しそのＩＤも付しました. ただし化学的な検討は行っておらず単純に残基1つ違う糖鎖を抽出しています. この検討に含まれる残基はHex, HexNAc, Fuc, NeuAc, NeuGc, 3-8NeuAc, 6NeuAC, 3-8NeuGc, 6NeuGcの9残基種になります. それぞれの構造に対して, 観測された質量と発現量, 発現量の系列ごとの平均, 標準偏差, CVなどを掲載しています.
...
- exp_list_zero_cut.csvの記述において組成に対するglyconnetへのハイパーリンクを追加しました. csvファイルを開きリンクをクリックするとglyconnectで組成を検索できます.
