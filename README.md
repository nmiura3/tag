...
- 今まではN型およびFOSとGSLとでは糖鎖リストの形式が異なっていましたが, 実は同じ形式でリストを用意すれば, N型でもGSLでも同じモジュールで実行できます. この際にはGSL糖鎖のリストでもA1カラムには”N”と入力必要があり, これによって正しいクラスを出力できませんでした. バージョンからは糖鎖リストのA2カラムに真のクラスを表す1文字を入力する事によって正しいクラスを出力ファイルに残す事が出来ます. これまで通りリストのA2カラムを空欄のまま実行してもエラーにはならないようにしました. 空欄の場合はN型と判断することにしてあります. これはHuman Glcyome Atrass Projectに納めるデータ形式として真のクラスを残すための対応になります.
