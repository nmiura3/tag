...
## 5.4が現在の最新版です.(5.4.1dになりました)
- TAGで読み込めるエクセルファイルは"xlsx"です. xlsのマスリストの場合は一回開いてxlsxで保存し直してください.
- 5.4.1dは現在windows版のみが提供されています.

## 1.  変更点
バージョン5.4では基本的に5.3と同様に動作させますが, 糖鎖解析の安定性と信頼性にかかる機能改善を行いました. 5.4.1のreadmeは[こちら](https://github.com/nmiura3/tag/blob/master/README_v5.4.1.md)

- 今まではN型およびFOSとGSLとでは糖鎖リストの形式が異なっていましたが, 実は同じ形式でリストを用意すれば, N型でもGSLでも同じモジュールで実行できます. この際にはGSL糖鎖のリストでもA1カラムには”N”と入力必要があり, これによって正しいクラスを出力できませんでした. バージョンからは糖鎖リストのA2カラムに真のクラスを表す1文字を入力する事によって正しいクラスを出力ファイルに残す事が出来ます. これまで通りリストB1カラムを空欄のまま実行してもエラーにはならないようにしました. 空欄の場合はN型と判断することにしてあります. これはHuman Glcyome Atrass Projectに納めるデータ形式として真のクラスを残すための対応になります.<br>
<img src="image/real_class_in_glycan_list.png" alt="center" width="320"/>

- 検量線の解析を行った際にR<sup>2</sup>が0.8より大きい糖鎖をexp_list_R2_0.8_listとう名前でリストとして出力するようにしました.
- 5.4.3cで出力するようにしたサマリーファイルに, サンプル中の検出率によってフィルターする機能を追加しました. これによって例えば50サンプル中1個だけのサンプルで検出できた糖鎖などをあらかじめ削除する事ができます. 詳細は後述.
exitの上のボタンをクリックすることによってこの機能を実現でき, 対象となるexp_list_zero_cut.csvファイルを選択すると, 閾値入力の窓が開きますのでここに閾値を入力してボタンをクリックしてください.<br>
<img src="image/tag_filter.png" alt="center" width="480"/>

- サマリーファイルを出力するようにしました. exp_list_mass_summary.tsv, exp_list_zero_cut_mass_summary.tsvを出力します. これらは, 同じスペクトルとして取得された糖鎖を１つのエントリーにまとめ, 平均などの統計値をエクセル関数を用いて記述しています. これによって, 行や列を削除した場合, 発現量の統計を自動で計算しなおすようになりました. また, 検量線の解析を行った場合は, 補完直線の傾き, 切片, 相関係数R<sup>2</sup>を含めました. さらにR<sup>2</sup>が0.8以上の糖鎖をTAG用のリストとしてexp_list_R2_0.8_list.csvという名前で出力しています.
- その他細かいバグフィックスを行いました. 
- 試験的に384スポットを一斉に解析できるようにしました. 動的にメモリを取得しますので, おそらくうまく動作すると思います.
- 解析後に出力されるexp_list.csvとexp_list_zero_cut.csvについて, 異性体をまとめるて1つのピークあたりの糖鎖組成のcandidateでまとめるフィルターを追加しました. これらはexp_list_mass.csvまたはexp_list_zero_cut_mass.csvという名前のファイルに出力されます. 
- exp_list.csv, exp_list_zero_cut.csvの表記で一部ずれている部分を修正しました.
- クラスタリングのアルゴリズムについて3番の方法を削除しました. 3番の機能については[こちら](https://github.com/nmiura3/tag/blob/master/README_v5.4.1c.md)


## 2. 概要
- windows版 tag_v5.4.1c_win.zip
  - MS windows11, windows10にて動作確認.
...
|filter with detection ratio |サマリーファイルを作成する際に, サンプル中にその糖鎖がどのくらいの比率で発現しているかによってその閾値を用いて糖鎖を削除する機能です. 科学的には, 恣意的にこのような削除を行う事は好ましくなく内容を吟味したのちに様々アクションするのが筋だと開発者は考えています. もともと検出率はサマリーファイルに出力しているので, 検出率が高いものだけエクセルで抽出するのは難しい事ではないのですが, あるユーザーたっての希望があり実装することにしました. これによっておこる不都合については開発者は責任を負いません. |
|exit|TAGを終了します.
...
| D1 | m/zとΔm/zによる分布をクラスター分析しどのクラスターを取得するかについてのフラグ. (5.4.1cからは9番をデフォルトとした)|
<br>
...
-帰属の際の理論的m/zとΔm/zの散布図からクラスタリングについて, どのクラスターを選ぶかによって結果が変わってきます. これは実際の質量分析における測定の精度にも強く依存します. どのクラスターを選ぶかについていくつかのオプションを用意しました. 
